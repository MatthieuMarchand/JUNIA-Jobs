                        ...
                        <div class="col-md-4">
                            ...
                            <select name="research_domain_names[]" id="research_domain_names" class="form-control" multiple>
                                @foreach ($domains as $domain)
                                    <option
                                        value="{{ $domain->name }}" {{ in_array($domain->name, request('research_domain_names', [])) ? 'selected' : '' }}>
                                        {{ $domain->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-4">
                            ...
                            <select name="contract_type_ids[]" id="contract_type_ids" class="form-control" multiple>
                                @foreach ($contractTypes as $contractType)
                                    <option
                                        value="{{ $contractType->id }}" {{ in_array($contractType->id, request('contract_type_ids', [])) ? 'selected' : '' }}>
                                        {{ $contractType->name }}
                                    </option>
                                @endforeach
                            </select>
                            ...
                            <div class="card-body">
                                <div class="d-flex mb-3">
                                    <a href="{{ route('companies.students.show', $student->id) }}" class="text-decoration-none d-flex">
                                        ...
                                        @if($photoUrl)
                                            <img src="{{ $photoUrl }}" alt="Photo de {{ $student->first_name }}" class="img-thumbnail"
                                ...
                                <div class="text-center mt-3">
                                    <a href="{{ route('companies.students.show', $student->id) }}" class="btn btn-outline-primary">Voir le profil
                                        complet</a>
                                </div>
                            </div>
                        </div>
